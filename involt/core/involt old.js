	...
	this.onReceiveParse = function(dataBlock){
		...
		for (var j=0; j<dataBlock.length; j++){
			...
			if(indexA == 0 && indexA < indexV && indexV == dataBlock[j].lastIndexOf('V') && indexV >= 0){
				...
				if(typeof involtListenForPin[index] === 'function') involtListenForPin(index, value);
			}
...
if (isSerial){
	...
	Involt.prototype.begin = function(){
		...
		var checkConnections = function(persistentDevices){
			if(persistentDevices){
				for(var k=0; k<persistentDevices.length; k++){
					involt.previousConnections[k] = persistentDevices[k].connectionId;
				};
			};
		};
	...
	Involt.prototype.connect = function(port, speed){
		...
		var onConnect = function(device){
			...
			console.log("Device connected:", port, " ID:", device.connectionId, device);

			involt.connection = device
